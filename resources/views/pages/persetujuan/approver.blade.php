...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
        <h1>Persetujuan - <span class="h5">Approver</span></h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <a href="{{ route('persetujuan') }}" class="btn btn-sm btn-danger btn-kembali"><i class="fas fa-arrow-left"></i> Kembali</a>
          </ol>
        </div>
      </div>
    </div>
...
<script>
  $(document).ready(function () {
    ...
    function approverData() {
      $.ajax({
        url: "{{ URL::route('persetujuan.approver.data') }}",
        type: 'GET',
        success: function (response) {
          let data_approve = ``;
          $.each(response.approvers, function (index, item) {
            data_approve += `
            <div class="col-lg-3 col-md-4 col-sm-12 col-12">
              <div class="card card-primary">
                ...
                <div class="card-body">`;
                  $.each(response.approve_alls, function (index_detail, item_detail) {
          ...
          $('.data-approve').html(data_approve);
    ...
    $('#btn-create').on('click', function() {
      $.ajax({
        url: "{{ URL::route('persetujuan.approver.create') }}",
        type: 'GET',
        success: function (response) {
          // role
    ...
    $(document).on('submit', '.form-create', function (e) {
      ...
      $.ajax({
        url: "{{ URL::route('persetujuan.approver.store') }}",
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
    ...
    $(document).on('change', '.select2', function () {
      ...
      timer = setTimeout(() => {
        ...
        $.ajax({
          url: "{{ URL::route('persetujuan.approver.update') }}",
          type: 'POST',
          data: formData,
          success: function (response) {
            Toast.fire({
    ...
    $(document).on('click', '#btn-approve-add', function (e) {
      ...
      $.ajax({
        url: "{{ URL::route('persetujuan.approver.add') }}",
        type: 'POST',
        data: formData,
        beforeSend: function () {
    ...
    $('body').on('click', '.btn-delete', function (e) {
      ...
      var url = '{{ route("persetujuan.approver.delete_all_btn", ":id") }}';
    ...
    $('#form-delete').submit(function (e) {
      ...
      $.ajax({
        url: "{{ URL::route('persetujuan.approver.delete_all') }}",
        type: 'POST',
        data: formData,
        beforeSend: function () {
    ...
    $('body').on('click', '.btn-delete-approve', function (e) {
      ...
      var id = $(this).attr('data-id');
      var url = '{{ route("persetujuan.approver.delete_btn", ":id") }}';
      url = url.replace(':id', id);
    ...
    $('#form-delete').submit(function (e) {
      ...
      $.ajax({
        url: "{{ URL::route('persetujuan.approver.delete') }}",
        type: 'POST',
        data: formData,
        beforeSend: function () {
          $('.btn-delete-spinner').css('display', 'block');
