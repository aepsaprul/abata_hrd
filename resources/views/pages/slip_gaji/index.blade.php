                          ...
                          <div class="btn-group">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                              @if (in_array("lihat_berkas", $current_data_navigasi))
                                <a href="{{ route('slip_gaji.cetak_pdf', [$item->id]) }}" class="dropdown-item text-indigo" target="_blank">
                                  <i class="fas fa-archive text-center mr-2" style="width: 20px;"></i> Lihat Berkas
                                </a>
                              @endif
                              @if (in_array("detail", $current_data_navigasi))
                                <a href="{{ route('slip_gaji.cetak_pdf_karyawan', [$item->id]) }}" class="dropdown-item text-indigo btn-detail" target="_blank">
                                  <i class="fas fa-eye text-center mr-2" style="width: 20px;"></i> Detail
                                </a>
                              @endif
                              @if (in_array("ubah", $current_data_navigasi))
                                <a href="#" class="dropdown-item btn-edit text-indigo" data-id="{{ $item->id }}">
                                  <i class="fas fa-pencil-alt text-center mr-2" style="width: 20px;"></i> Ubah
                                </a>
                              @endif
                              @if (in_array("hapus", $current_data_navigasi))
                                <a href="{{ route('slip_gaji.delete', [$item->id]) }}" class="dropdown-item btn-delete text-indigo" onclick="return confirm('Yakin akan dihapus?')">
      ...
      <form id="form-edit" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control form-control-sm" id="edit_tahun" name="edit_tahun" required>
          </div>
          <div class="mb-3">
            <label for="edit_bulan" class="form-label">Bulan</label>
            <input type="text" class="form-control form-control-sm" id="edit_bulan" name="edit_bulan" maxlength="10" required>
          </div>
          <div class="mb-3">
            <label for="edit_periode" class="form-label">Periode</label>
            <input type="text" class="form-control form-control-sm" id="edit_periode" name="edit_periode" maxlength="50" required>
          </div>
        </div>
