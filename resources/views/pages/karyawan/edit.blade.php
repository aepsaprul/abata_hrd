                      ...
                      <div class="card-body">
                          <div class="tab-content">
                              ...
                              <div class="active tab-pane" id="biodata">
                                <form id="biodata_form" method="POST" enctype="multipart/form-data" class="mb-3">
                                  <div class="row">
                                    <div class="col-md-3">
                                      {{-- id --}}
                                          ...
                                          <div class="text-center profile_img">
                                            @if ($karyawan->foto)
                                              ...
                                              {{-- @if(file_exists('image/' . $karyawan->foto)) --}}
                                                <img class="profile-user-img img-fluid" src="{{ asset(env('APP_URL_IMG') . 'image/' . $karyawan->foto) }}" alt="User profile picture" style="width: 100%;">
                                              @else
                  ...
                  <div class="card-body">
                    <div class="row mb-3">
                      <table class="table table-bordered">
                        ...
                        <tbody id="tabel_body_histori">
                          @foreach ($histori_jabatans as $histori_jabatan)
                            <tr class="tabel_tr_histori">
                              <td>{{ $histori_jabatan->dataJabatan->nama_jabatan }}</td>
                              <td class="text-center">{{ tglCarbon($histori_jabatan->tanggal, 'd/m/Y') }}</td>
                              <td class="text-center">
                                <button class="btn btn-danger btn_hapus_histori" data-id={{ $histori_jabatan->id }}><i class="fas fa-times" data-id={{ $histori_jabatan->id }}></i></button>
                              </td>
                            </tr>
                          @endforeach
                        </tbody>
                    ...
                    <div class="row mb-3">
                      <div class="col-6">
                        <label for="histori_jabatan">Jabatan</label>
                        <select name="histori_jabatan" id="histori_jabatan" class="form-control">
                      ...
                      <div class="col-6">
                        <label for="histori_tanggal">Tanggal</label>
                        <input type="date" name="histori_tanggal" id="histori_tanggal" class="form-control" value="{{ date('Y-m-d') }}">
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-12">
                        <button class="btn btn-primary btn-histori-spinner d-none" disabled style="width: 130px;">
                          <span class="spinner-grow spinner-grow-sm"></span>
                          Loading...
                        </button>
                        <button id="btn_histori_jabatan" class="btn btn-primary btn-histori-save" style="width: 130px;"><i class="fas fa-save"></i> Simpan</button>
                      </div>
                    </div>
                  </div>
                </div>
...
<script type="text/javascript">
    ...
    $('#btn_histori_jabatan').on('click', function(e) {
      e.preventDefault();
      const id = $('#id').val();
      const jabatan = $('#histori_jabatan').val();
      const tanggal = $('#histori_tanggal').val();
      
      $.ajax({
        url: "{{ route('karyawan.histori.jabatan.store') }}",
        type: "post",
        data: {
          master_karyawan_id: id,
          master_jabatan_id: jabatan,
          tanggal: tanggal
        },
        beforeSend: function () {
          $('.btn-histori-spinner').removeClass('d-none');
          $('.btn-histori-save').addClass('d-none');
        },
        success: function(response) {
          Toast.fire({
            icon: 'success',
            title: 'Data histori berhasil diperbaharui'
          });
          $('.btn-histori-spinner').addClass('d-none');
          $('.btn-histori-save').removeClass('d-none');
        }
      })
    })

    // hapus histori
    $('#tabel_body_histori').on('click', '.btn_hapus_histori', function(e) {
      // Menghapus elemen .form-group terdekat
      $(this).closest('#tabel_body_histori .tabel_tr_histori').remove();

      const id = e.target.dataset.id;
      let url = "{{ route('karyawan.histori.jabatan.delete', [':id']) }}";
      url = url.replace(':id', id);
      ...
      $.ajax({
        ...
        success: function(response) {
          Toast.fire({
            icon: 'success',
            title: 'Data histori berhasil dihapus'
          });
        }
      })
    });
