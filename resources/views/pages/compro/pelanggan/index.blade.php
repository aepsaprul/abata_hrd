                    ...
                    <div class="card">
                        {{-- @if (in_array("tambah", $current_data_navigasi)) --}}
                            <div class="card-header">
                              <form action="{{ route('compro.pelanggan.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                ...
                                <tbody>
                                    @foreach ($pelanggans as $key => $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->grup }}</td>
                                            <td>{{ $item->nama }}</td>
                                            <td><img src="{{ url(env('APP_URL_IMG') . 'img_compro/pelanggan/' . $item->gambar) }}" alt="pelanggan_gambar" style="max-width: 100px;"></td>
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right">
                                                            {{-- @if (in_array("ubah", $current_data_navigasi)) --}}
                                                                <a
                                                                    href="{{ route('compro.pelanggan.edit', [$item->id]) }}"
...
<script>
    $(document).ready(function () {
        ...
        $('#form-delete').submit(function (e) {
            ...
            $.ajax({
                url: "{{ URL::route('compro.pelanggan.delete') }}",
                type: 'POST',
                data: formData,
                beforeSend: function () {
